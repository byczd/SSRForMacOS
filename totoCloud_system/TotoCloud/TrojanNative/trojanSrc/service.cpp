...
Service::Service(Config &config, const string &certfile, bool test) :
    config(config),
...
//        }
    ...
    if (config.run_type == Config::SERVER) {
        ...
        if (config.ssl.sni.empty()) {
            config.ssl.sni = config.remote_addr; //如果为空的话，就赋值为服务器host
        }
        ...
        if (config.ssl.verify) {
            ...
            if (config.ssl.cert.empty()) {
                ssl_context.set_default_verify_paths(); //配置上下文以使用默认目录查找证书颁发机构证书.
                
//检查是否定义了宏： __WIN32__ 如果定义了则是windows，否则不是
#ifdef _WIN32
...
//检查是否定义了宏： __APPLE__ 如果定义了则是mac，否则不是
#ifdef __APPLE__
//进一步判断mac和iOS，两者上处理方式不一样
...
//  iOS系统需要提供证书文件
        ...
        // is Mac
                ...
                if (!certfile.empty()) {
                    string filepath=string(certfile+"/"+config.ssl.cert);
                    ssl_context.load_verify_file(filepath);
                }else{
            ...
            ssl_context.set_verify_mode(verify_none); //verify=false
        }
...
//        用SSL_CTX_set_cipher_list可以指定自己希望用的算法(实际上只是提高其优先级,是否能使用还要看对方是否支持)。
    }
