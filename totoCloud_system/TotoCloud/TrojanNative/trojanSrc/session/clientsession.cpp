...
void ClientSession::out_async_read() {
    ...
    out_socket.async_read_some(boost::asio::buffer(out_read_buf, MAX_LENGTH), [this, self](const boost::system::error_code error, size_t length) {
        if (error) {
            string msg(error.message());//“end of file” 或 "Operation canceled"    
            printf("[trojanClient] recv from trojanServer err: %s",msg.c_str());
            destroy();
            return;
        }
...
void ClientSession::in_recv(const string &data) {
    switch (status) {
        ...
        case REQUEST: {
            ...
            break;
        }//ut_write_buf = "8efc3abcd2e1760decca9cbd133e783edd2cd864d3f3bbe641cfd8dd\r\n\U00000001\U00000003\U00000012www.epochtimes.com\U00000001\xbb\r\n"
        case CONNECT: {
            sent_len += data.length();
            first_packet_recv = true;
            out_write_buf += data;//原来trojan协议的密码是加密后的密码
            break;
        }
        case FORWARD: {
            sent_len += data.length();
            ...
            resolver.async_resolve(config.remote_addr, to_string(config.remote_port), [this, self](const boost::system::error_code error, const tcp::resolver::results_type& results) {
                ...
                out_socket.next_layer().async_connect(*iterator, [this, self](const boost::system::error_code error) {
                    ...
                    out_socket.async_handshake(stream_base::client, [this, self](const boost::system::error_code error) {
                        if (error) {//与trojanServer握手失败
                            string remoteAdrr(config.remote_addr);
                            string remotePort(to_string(config.remote_port));
                            string msg(error.message());//"certificate verify failed (SSL routines, tls_process_server_certificate)"
//                            printf("[trojanClient] SSL handshake failed with [%s:%s]： %s",remoteAdrr.c_str(),remotePort.c_str(),msg.c_str());
